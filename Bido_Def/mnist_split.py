...
def target_train(train_loader, target_model, optimiser):
    ...
    for batch, (X, Y) in enumerate(tqdm(train_loader)):
        ...
        #optimizer.zero_grad()
        bido_loss_lst.append(loss_bido.item())
        #print(bido_loss_lst[0])
        loss=bido_loss_lst[0]+loss_target
        loss.backward()
        optimiser.step()
...
##torch.save(attack_model, '/vast/home/sdibbo/def_ddlc/model_attack/MNIST_20_epoch_CNN_linear_attack.pt')
#torch.save(target_model, '/vast/home/sdibbo/def_ddlc/model_target/MNIST_20_epoch_CNN_linear_target.pt')

#df = pd.DataFrame(list(zip(*[psnr_lst,  ssim_lst, fid_lst]))).add_prefix('Col')

#df.to_csv('/vast/home/sdibbo/def_ddlc/result/MNIST_20_epoch_CNN_attack_linear.csv', index=False)