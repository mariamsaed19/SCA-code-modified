...
def target_train(train_loader, target_model, optimiser):
    ...
    for batch, (X, Y) in enumerate(tqdm(train_loader)):
        ...
        loss_bido, cross_loss, out_digit, hx_l_list, hy_l_list = multilayer_hsic(target_model, cost, X, Y, a1=1.0, a2=1.0,
                                                                            n_classes=6, ktype='gaussian', hsic_training=True,measure='HSIC')
        #optimizer.zero_grad()
...
def attack_test(train_loader, target_model, attack_model):
    ...
    for batch, (data, targets) in enumerate(tqdm(train_loader)):
        ...
        if (psnr_val>15.00):
            ...
            plt.draw()
            plt.savefig(f'/dartfs-hpc/rc/home/h/f0048vh/Sparse_guard/Bido_def/plot/org_img{batch}.jpg', dpi=100, bbox_inches='tight')
            plt.imshow(recreated_data[0][0].cpu().detach().numpy(), cmap='gray')
            plt.xticks([])
            plt.yticks([])
            #plt.imshow(mfcc_spectrogram[0][0,:,:].numpy(), cmap='viridis')
            plt.draw()
            plt.savefig(f'/dartfs-hpc/rc/home/h/f0048vh/Sparse_guard/Bido_def/plot/gen_img{batch}.jpg', dpi=100, bbox_inches='tight')
