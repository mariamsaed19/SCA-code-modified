...
train_data= torchvision.datasets.CIFAR10('/dartfs-hpc/rc/home/h/f0048vh/Sparse_guard/data', train=True, download=True,
                             transform=transform)
sampler_data = list(range(40000, len(train_data), 1))
train_subset1 = torch.utils.data.Subset(train_data, range(50000))
train_subset2 = torch.utils.data.Subset(train_data, range(30000))
...
#for data, targets in enumerate(tqdm(train_loader)):
    ...
    # Reset gradients
        ...
        target_outputs2 = clf_model.first_part(data)
        #target_outputs= target_outputs.view(-1,32*32*500)
        target_outputs2 = clf_model.second_part(target_outputs2)
        attack_outputs2 = attack_model(target_outputs2)
        #target_outputs= target_outputs.view(-1,32*32*500)
        #clf_outputs = clf_model.second_part(clf_outputs)        
        # Next, recreate the data with the attacker
        #clf_outputs = clf_outputs.view(1, 32, 1, 16*16)
