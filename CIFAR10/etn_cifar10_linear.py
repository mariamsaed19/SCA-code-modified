...
def attack_test(train_loader, target_model, attack_model):
    ...
    for batch, (data, targets) in enumerate(tqdm(train_loader)):
        ...
        plt.draw()
        plt.savefig(f'./etn_plot/CIFAR/linear/org_img{batch}.jpg', dpi=100, bbox_inches='tight')
        plt.imshow((recon_img.cpu().detach().numpy()))
        plt.xticks([])
        plt.yticks([])
        #plt.imshow(mfcc_spectrogram[0][0,:,:].numpy(), cmap='viridis')
        plt.draw()
        plt.savefig(f'./etn_plot/CIFAR/linear/recon_img{batch}.jpg', dpi=100, bbox_inches='tight')
...
torch.save(attack_model, '/vast/home/sdibbo/def_ddlc/model_attack/etn/CIFAR10_20_epoch_CNN_linear_attack.pt')
torch.save(target_model, '/vast/home/sdibbo/def_ddlc/model_target/etn/CIFAR10_20_epoch_CNN_linear_target.pt')

df = pd.DataFrame(list(zip(*[psnr_lst,  ssim_lst, fid_lst]))).add_prefix('Col')

df.to_csv('/vast/home/sdibbo/def_ddlc/result/etn/CIFAR10_20_epoch_CNN_attack_linear.csv', index=False)