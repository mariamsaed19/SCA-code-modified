...
data_loader = torch.utils.data.DataLoader(torchvision.datasets.FashionMNIST('data', train=True, download=True, transform=transform),
...
torch.save(attack_model, '/vast/home/sdibbo/def_ddlc/model_attack/etn/gan_FMNIST_20_epoch_CNN_linear_attack.pt')
torch.save(target_model, '/vast/home/sdibbo/def_ddlc/model_target/etn/gan_FMNIST_20_epoch_CNN_linear_target.pt')

df = pd.DataFrame(list(zip(*[psnr_lst,  ssim_lst, fid_lst]))).add_prefix('Col')

df.to_csv('/vast/home/sdibbo/def_ddlc/result/etn/gan_FMNIST_20_epoch_CNN_attack_linear.csv', index=False)