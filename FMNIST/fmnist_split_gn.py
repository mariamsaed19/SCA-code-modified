...
def attack_test(train_loader, target_model, attack_model):
    ...
    for batch, (data, targets) in enumerate(tqdm(train_loader)):
        ...
        ### Commented to skip saving images
        '''
...
torch.save(attack_model, '/vast/home/sdibbo/def_ddlc/model_attack/FMNIST_20_epoch_CNN_noise_linear_attack.pt')
torch.save(target_model, '/vast/home/sdibbo/def_ddlc/model_target/FMNIST_20_epoch_CNN_noise_linear_target.pt')

df = pd.DataFrame(list(zip(*[psnr_lst,  ssim_lst, fid_lst]))).add_prefix('Col')

df.to_csv('/vast/home/sdibbo/def_ddlc/result/FMNIST_20_epoch_CNN_attack_noise_linear.csv', index=False)