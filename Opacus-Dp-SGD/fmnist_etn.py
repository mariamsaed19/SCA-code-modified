...
train_loader = torch.utils.data.DataLoader(datasets.FashionMNIST('/dartfs-hpc/rc/home/h/f0048vh/opacus/data', train=True, download=True,
...
test_loader = torch.utils.data.DataLoader(datasets.FashionMNIST('/dartfs-hpc/rc/home/h/f0048vh/opacus/data', train=False, 
              transform=transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.1307,), 
...
def attack_test(train_loader, model, attack_model):
    ...
    for batch, (data, targets) in enumerate(tqdm(train_loader)):
        ...
        plt.draw()
        plt.savefig(f'/dartfs-hpc/rc/home/h/f0048vh/opacus/org/org_img{batch}.jpg', dpi=100, bbox_inches='tight')
        plt.imshow(recreated_data[0][0].cpu().detach().numpy(), cmap='gray')
        plt.xticks([])
        plt.yticks([])
        #plt.imshow(mfcc_spectrogram[0][0,:,:].numpy(), cmap='viridis')
        plt.draw()
        plt.savefig(f'/dartfs-hpc/rc/home/h/f0048vh/opacus/recon/recon_img{batch}.jpg', dpi=100, bbox_inches='tight')
        '''
