...
#for data, targets in enumerate(tqdm(train_loader)):
    ...
    # Reset gradients
        ...
        target_outputs = target_outputs.view(1, 32, 2, 16*16)
...
def attack_test(train_loader, model, attack_model):
    ...
    for batch, (data, targets) in enumerate(tqdm(train_loader)):
        ...
        ### For Saving recon images, uncomment below code blocks
            ...
            plt.draw()
            plt.savefig(f'/dartfs-hpc/rc/home/h/f0048vh/opacus/plot/org_img{batch}.jpg', dpi=100, bbox_inches='tight')
            plt.imshow((recon_img.cpu().detach().numpy()))
            plt.xticks([])
            plt.yticks([])
            #plt.imshow(mfcc_spectrogram[0][0,:,:].numpy(), cmap='viridis')
            plt.draw()
            plt.savefig(f'/dartfs-hpc/rc/home/h/f0048vh/opacus/plot/recon_img{batch}.jpg', dpi=100, bbox_inches='tight')
