...
def attack_test(train_loader, target_model, attack_model):
    ...
    for batch, (data, targets) in enumerate(tqdm(train_loader)):
        ...
        #plt.imshow(mfcc_spectrogram[0][0,:,:].numpy(), cmap='viridis')
        plt.draw()
        plt.savefig(f'./result/etn/plot/recon_lca2_larger_batch_img{batch}.jpg', dpi=100, bbox_inches='tight')
...
print("**********Test Starting************")
torch.save(attack_model, './result/etn/MNIST_50_epoch_CNN_lca2_larger_batch_attack.pt')
torch.save(target_model, './result/etn/MNIST_25_epoch_CNN_lca2_larger_batch_target.pt')
...
df.to_csv('./result/etn/MNIST_20_epoch_CNN_attack_lca2_larger_batch.csv', index=False)