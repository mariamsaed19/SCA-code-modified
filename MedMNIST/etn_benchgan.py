...
'''
sampler_data = list(range(40000, len(dataset), 1))
train_subset1 = torch.utils.data.Subset(dataset, range(40000))
train_subset2 = torch.utils.data.Subset(dataset, sampler_data)
...
def generator_train_step(batch_size, discriminator, generator, g_optimizer, cost):
    ...
    z = Variable(torch.randn(batch_size, 36)).to(device)
    fake_labels = Variable(torch.LongTensor(np.random.randint(0, 6, batch_size))).to(device)
    fake_images = generator(z, fake_labels)
    validity = discriminator(fake_images, fake_labels)
    g_loss = cost(validity, Variable(torch.ones(batch_size)).to(device))
    g_loss.backward()
    g_optimizer.step()
    return g_loss.item()
...
def discriminator_train_step(batch_size, discriminator, generator, d_optimizer, cost, real_images, labels):
    ...
    real_loss = cost(real_validity, Variable(torch.ones(batch_size)).to(device))
    ...
    fake_loss = cost(fake_validity, Variable(torch.zeros(batch_size)).to(device))
    ...
    for i, (images, labels) in enumerate (tqdm(data_loader)):
       ...
       # writer.add_scalars('scalars', {'g_loss': g_loss, 'd_loss': (d_loss / n_critic)}, step)  
        ...
        if step % display_step == 0:
            generator.eval()
            z = Variable(torch.randn(5, 36)).to(device)
...
def generate_img(generator, digit):
    z = Variable(torch.randn(1, 36)).to(device)
...
def attack_test(train_loader, target_model, attack_model):
    ...
    for batch, (data, targets) in enumerate(tqdm(train_loader)):
        ...
        print('FID is: %.3f' % fid_val)        #gen_data= recreated_data.view(-1,32*32*3)
        ...
        if (psnr_val>15.00):
            ...
            plt.draw()
            plt.savefig(f'/dartfs-hpc/rc/home/h/f0048vh/Sparse_guard/MedMNIST/plot/ganp/org_img{batch}.jpg', dpi=100, bbox_inches='tight')
            plt.imshow(recreated_data[0][0].cpu().detach().numpy(), cmap='gray')
            plt.xticks([])
            plt.yticks([])
            #plt.imshow(mfcc_spectrogram[0][0,:,:].numpy(), cmap='viridis')
            plt.draw()
            plt.savefig(f'/dartfs-hpc/rc/home/h/f0048vh/Sparse_guard/MedMNIST/plot/ganp/gen_img{batch}.jpg', dpi=100, bbox_inches='tight')
