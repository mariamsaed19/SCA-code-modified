...
def attack_test(train_loader, target_model, attack_model):
    ...
    for batch, (data, targets) in enumerate(tqdm(train_loader)):
        ...
        if (psnr_val>15.00):
            ...
            plt.draw()
            plt.savefig(f'/dartfs-hpc/rc/home/h/f0048vh/Sparse_guard/MedMNIST/plot/ganw/org_img{batch}.jpg', dpi=100, bbox_inches='tight')
            plt.imshow(recreated_data[0][0].cpu().detach().numpy(), cmap='gray')
            plt.xticks([])
            plt.yticks([])
            #plt.imshow(mfcc_spectrogram[0][0,:,:].numpy(), cmap='viridis')
            plt.draw()
            plt.savefig(f'/dartfs-hpc/rc/home/h/f0048vh/Sparse_guard/MedMNIST/plot/ganw/gen_img{batch}.jpg', dpi=100, bbox_inches='tight')
