...
train_loader = DataLoader(train_set,   batch_size=batch_size_train, shuffle=True,sampler=torch.utils.data.distributed.DistributedSampler(train_set, num_replicas=world_size, rank=rank))
test_loader = DataLoader(test_set, batch_size=batch_size_test, shuffle=True,sampler=torch.utils.data.distributed.DistributedSampler(test_set, num_replicas=world_size, rank=rank))

train_set_other, test_set_other = torch.utils.data.random_split(face_dataset,
                                                   [1000,201599])
attack_test_loader = DataLoader(train_set_other,   batch_size=batch_size_attack_test, shuffle=True,sampler=torch.utils.data.distributed.DistributedSampler(train_set_other, num_replicas=world_size, rank=rank))  
...
#for data, targets in enumerate(tqdm(train_loader)):
    ...
    # Reset gradients
        ...
        attack_outputs = attack_model.module(target_outputs)
