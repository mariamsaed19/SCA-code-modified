...
#for data, targets in enumerate(tqdm(train_loader)):
    ...
    # Reset gradients
        ...
        target_outputs = target_outputs.view(1, 2*data.shape[0], 16, 16*16)
        ...
        attack_outputs = attack_outputs.repeat(data.shape[0], 1, int(data.shape[0]/8), 1)
        ...
        clf_outputs = clf_outputs.view(1, 2*data.shape[0], 16, 16*16)
        #target_outputs= target_outputs.view(-1,32*32*500)
        #clf_outputs = clf_model.second_part(clf_outputs)        
        # Next, recreate the data with the attacker
        #print(clf_outputs.shape)      
        attack_outputs2 = attack_model(clf_outputs)
        attack_outputs2 = attack_outputs2.repeat(data.shape[0], 1, int(data.shape[0]/8), 1)
        # We want attack outputs to resemble the original data
...
def attack_test(train_loader, target_model, attack_model):
    ...
    for batch, (data, targets) in enumerate(tqdm(train_loader)):
        ...
        if (data.shape[0]!=32):
           ...
           target_outputs = target_outputs.view(1,data.shape[0],32, 16*16)
        ...
        recreated_data = attack_model(target_outputs)
        #print(recreated_data.shape)
        recreated_data = recreated_data.repeat(data.shape[0], 1,1, 1)
        ...
        ### For Saving recon images, uncomment below code blocks
            ...
            #print(recon_img.shape)
            plt.draw()
            plt.savefig(f'/dartfs-hpc/rc/home/h/f0048vh/Sparse_guard/celeba/plot/titcomb/org_img{batch}.jpg', dpi=100, bbox_inches='tight')
            plt.imshow((recon_img.cpu().detach().numpy()))
            plt.xticks([])
            plt.yticks([])
            #plt.imshow(mfcc_spectrogram[0][0,:,:].numpy(), cmap='viridis')
            plt.draw()
            plt.savefig(f'/dartfs-hpc/rc/home/h/f0048vh/Sparse_guard/celeba/plot/titcomb/recon_img{batch}.jpg', dpi=100, bbox_inches='tight')
